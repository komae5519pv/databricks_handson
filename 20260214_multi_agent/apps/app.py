...
# エージェントクライアントの初期化
AGENT_ENDPOINT = "https://adb-984752964297111.11.azuredatabricks.net/serving-endpoints/mas-93856fe9-endpoint/invocations"
agent = AgentClient(AGENT_ENDPOINT)

# Add default welcome text that can be customized
DEFAULT_WELCOME_TITLE = "在庫確認アシスタント"
DEFAULT_WELCOME_DESCRIPTION = "アパレルブランドショップ店舗スタッフ向けの在庫確認システムです。商品IDや在庫状況について質問してください。"

# Add default suggestion questions
DEFAULT_SUGGESTIONS = [
    "都内で商品ID 1008 ある？在庫がなければ最短入荷も教えて。色はネイビーです",
    "新宿店で取り置き予約できる？",
    "銀座店の在庫状況を教えて",
    "商品ID 2001 の在庫はどこにある？"
]
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    html.Div("在庫確認アシスタントとの会話", className="sidebar-header-text"),
        ...
        html.Div([
            html.Div("在庫確認アシスタント", id="logo-container", className="logo-container")
        ...
        html.Div([
            ...
            html.A(
                ...
                href=f"https://{os.getenv('DATABRICKS_HOST', 'adb-984752964297111.11.azuredatabricks.net')}/login.html",
        ...
        html.Div([
            ...
            html.Div([
                ...
                className="genie-logo-container-header"),
                    ...
                    dbc.Modal([
                        dbc.ModalHeader(dbc.ModalTitle("ウェルカムメッセージのカスタマイズ")),
                        dbc.ModalBody([
                            html.Div([
                                html.Label("ウェルカムタイトル", className="modal-label"),
                                dbc.Input(
                                    id="welcome-title-input",
                                    type="text",
                                    placeholder="ウェルカムメッセージのタイトルを入力してください",
                                    className="modal-input"
                                ),
                                html.Small(
                                    "このタイトルはウェルカム画面の上部に表示されます",
                                    className="text-muted d-block mt-1"
                                )
                            ], className="modal-input-group"),
                            html.Div([
                                html.Label("ウェルカム説明", className="modal-label"),
                                dbc.Textarea(
                                    id="welcome-description-input",
                                    placeholder="アプリケーションの目的を理解するのに役立つ説明を入力してください",
                                    className="modal-input",
                                    style={"height": "80px"}
                                ),
                                html.Small(
                                    "この説明はタイトルの下に表示され、ユーザーのガイドに役立ちます",
                                    className="text-muted d-block mt-1"
                                )
                            ], className="modal-input-group"),
                            html.Div([
                                html.Label("サジェスション質問", className="modal-label"),
                                html.Small(
                                    "ウェルカム画面に表示される4つのサジェスション質問をカスタマイズしてください",
                                    className="text-muted d-block mb-3"
                                ),
                                dbc.Input(
                                    id="suggestion-1-input",
                                    type="text",
                                    placeholder="1番目のサジェスション質問",
                                    className="modal-input mb-2"
                                ),
                                dbc.Input(
                                    id="suggestion-2-input",
                                    type="text",
                                    placeholder="2番目のサジェスション質問",
                                    className="modal-input mb-2"
                                ),
                                dbc.Input(
                                    id="suggestion-3-input",
                                    type="text",
                                    placeholder="3番目のサジェスション質問",
                                    className="modal-input mb-2"
                                ),
                                dbc.Input(
                                    id="suggestion-4-input",
                                    type="text",
                                    placeholder="4番目のサジェスション質問",
                                    className="modal-input"
                                )
                            ], className="modal-input-group")
                        ]),
            ...
            html.Div([
                html.Div([
                    dcc.Input(
                        id="chat-input-fixed",
                        placeholder="商品IDや在庫について質問してください...",
...
def call_llm_for_insights(df, prompt=None):
    ...
    try:
        ...
        response = client.serving_endpoints.query(
            os.getenv("DATABRICKS_ENDPOINT_NAME"),
            messages=messages,
        )
...
def get_model_response(trigger_data, current_messages, chat_history):
    ...
    try:
        # Convert current messages to agent format
        messages_for_agent = []
        
        # Debug: Print current_messages structure
        print(f"DEBUG: current_messages type: {type(current_messages)}")
        print(f"DEBUG: current_messages length: {len(current_messages) if isinstance(current_messages, list) else 'N/A'}")
        print(f"DEBUG: current_messages: {current_messages}")
        
        # For now, just send the current user input
        messages_for_agent = [{"role": "user", "content": user_input}]
        
        print(f"DEBUG: messages_for_agent: {messages_for_agent}")
        
        response = agent.query(messages_for_agent)
        
        # Create bot response with markdown formatting
        bot_response = html.Div([
            html.Div([
                html.Div(className="model-avatar"),
                html.Span("在庫確認アシスタント", className="model-name")
            ], className="model-info"),
            html.Div([
                dcc.Markdown(response, className="message-text")
        ...
        error_msg = f"申し訳ありません。エラーが発生しました: {str(e)}。もう一度お試しください。"
